...
path1 = "/home/rick/Documents/research/SRSS/data/ICRA_data/dynamical_tasks/task_low_energy"
...
for file in files1:
	if not os.path.isdir(file):
		step,tmpTime,energy,energy_sum,z,x = [],[],[],[],[],[]
		f = open(path1 + "/" + file, "r");
		i = 0
		for line in f:
			i += 1
			...
			step.append(value[0])
			tmpTime.append(value[1])
			energy.append(float(value[2:]))
			energy_sum.append(sum(float(value[2:]))/20)

		time.append(datetime.strptime(tmpTime[i], "%M:%S.%f") - datetime.strptime(tmpTime[0], "%M:%S.%f")).seconds
